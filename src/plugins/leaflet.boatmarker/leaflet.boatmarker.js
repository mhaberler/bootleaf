...
L.BoatMarker = L.Marker.extend({
	...
	setStyle: function(style) {
		this.options.icon.setStyle(style);
	},
...
L.boatMarker = function(pos, options) {
	...
	var c = ("color" in options) ? options.fillColor : "#f1c40f";
	c = ("fillColor" in options) ? options.fillColor : c;
	var i = ("idleCircle" in options) ? options.idleCircle : false;
	options.icon = new L.BoatIcon({ fillColor: c, idleCircle: i});

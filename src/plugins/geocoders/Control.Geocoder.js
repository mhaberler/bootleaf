var leafletControlGeocoder = (function (L) {
  ...
  var WmoId = /*#__PURE__*/function () {
    function WmoId(options) {
      this.options = {
        next: undefined,
        summary: {},
        sizeInMeters: 10000
    ...
    var _proto = WmoId.prototype;

    _proto.geocode = function geocode(query, cb, context) {
      //console.log("wmoid.geocode", query);
      var found = [];
      var regex = new RegExp(query, "i"); //@ts-ignore

      for (var i in this.options.summary.features) {
        //@ts-ignore
        var p = this.options.summary.features[i].properties;
